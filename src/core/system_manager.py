    ...
    def _initialize_qt(self) -> bool:
        ...
        try:
            ...
            if settings.ui.fullscreen:
                # Try to disable window context help button if the attribute exists
                try:
                    if hasattr(
                        Qt.ApplicationAttribute, "AA_DisableWindowContextHelpButton"
                    ):
                        attr = Qt.ApplicationAttribute.AA_DisableWindowContextHelpButton
                        self.app.setAttribute(attr)
                except (AttributeError, TypeError):
                    # Attribute doesn't exist in this PyQt6 version, skip it
                    pass
    ...
    def _initialize_main_window(self) -> bool:
        ...
        try:
            ...
            self.main_window = MainWindow(
                camera_service=self.camera_service, sync_service=self.sync_service
            )
